...
export const action = async ({ request }: any) => {
  ...
  console.log("form=", createToken, deleteToken);
  const session = await getSession(request.headers.get("Cookie"));
  const accessToken = session.get("accessToken");
  let newToken;
  let removeToken;
                  ...
                  <>
                    {data.map((token: any, index: number) => (
                      <tr
                        ...
                        key={index}
                      >
                        <td className="text-lg">{token.uuid}</td>
              ...
              >
                ...
                {actionData && actionData.newToken && (
                  <div className="flex flex-col gap-y-6 pt-8">
                    <label htmlFor="keyid" className="pb-1">
                      API Key ID
                      <div className="input-icons">
                        ...
                        <input
                          ...
                          required
                          disabled
                          className="input-field rounded !w-full truncate"
                        />
                        <i>
                          <Copy
