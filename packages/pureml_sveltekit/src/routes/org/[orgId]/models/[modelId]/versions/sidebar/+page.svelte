...
<div class="flex gap-x-4">
  ...
  <Select intent="primary" fullWidth={false} name="version" title={_version}>
    <form method="POST" action="?/changeVersion" use:enhance class="w-full">
      <input type="hidden" name="versionSelected" value={_version} />
  ...
  <div class="flex flex-col gap-y-4 mt-6">
    ...
    <div class="flex gap-x-4">
      <Select intent="primary" fullWidth={true} name="branch" title={_branch}>
        ...
        {/each}</Select
      >
      <Select intent="primary" fullWidth={false} name="version" title="version">
        {#each versionData as version}
          <ListboxOption
            value={version.version}
            class="flex items-center justify-between px-4 py-2 rounded-md text-base text-slate-600 font-medium cursor-pointer hover:bg-slate-100 hover:border-none focus:outline-none"
