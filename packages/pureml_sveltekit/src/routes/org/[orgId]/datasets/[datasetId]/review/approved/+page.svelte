...
<svelte:head>
  <title>{`Approved Commits | ${$page.params.datasetId}`}</title>
</svelte:head>

<Tabbar tabType="tertiary" tabContent="datasetReviewTab" tab="approved" />
      ...
      {#if data.allCommits}
        {#each data.allCommits as reviewCommit}
          {#if reviewCommit.is_complete && reviewCommit.is_accepted}
            <a
              href={`/org/${data.orgDetails[0].name}/datasets/${reviewCommit.dataset.name}/versions/main/performance`}
              type="submit"
              class="pb-6 w-full"
            >
              <div
