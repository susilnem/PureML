...
<svelte:head>
  <title>{`Rejected Commits | ${$page.params.datasetId}`}</title>
</svelte:head>

<Tabbar tabType="tertiary" tabContent="datasetReviewTab" tab="rejected" />
      ...
      {#if data.allCommits}
        {#each data.allCommits as reviewCommit}
          {#if reviewCommit.is_complete && !reviewCommit.is_accepted}
            <a
              href={`/org/${data.orgDetails[0].name}/datasets/${reviewCommit.dataset.name}/review/${reviewCommit.uuid}/rejected`}
              type="submit"
              class="pb-6 w-full"
            >
              <div
              ...
              >
                <div class="flex items-center">
                  ...
                  <div class="text-left text-slate-600 px-4">
                    ...
                    <a
                      href={`/org/${data.orgDetails[0].name}/datasets/${reviewCommit.dataset.name}`}
                      class="font-medium text-slate-800"
                    >
                      {reviewCommit.dataset.name}
                    </a>{" "}
